...
import { useSendOtp, useVerifyOtp, OtpSendInputType } from "@workspace/api-client-react";
    ...
    verifyOtp.mutate(
      { data: { contact, otp } },
      {
        onSuccess: (data) => {
          ...
          else setLocation("/privacy"); // dashboard or privacy
        }
      }
    );
  };
        ...
        <div className="mx-auto w-full max-w-sm lg:w-96">
          ...
          {step === "contact" ? (
            <form onSubmit={handleSendOtp} className="space-y-6">
              ...
              <Button type="submit" className="w-full h-12 text-base" disabled={sendOtp.isPending || !contact}>
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-6">
              ...
              <Button type="submit" className="w-full h-12 text-base" disabled={verifyOtp.isPending || otp.length !== 6}>
                {verifyOtp.isPending ? <Loader2 className="mr-2 h-5 w-5 animate-spin" /> : null}
                Verify & Login
              </Button>
              <div className="text-center">
                <button type="button" onClick={() => setStep("contact")} className="text-sm font-medium text-primary hover:underline">
